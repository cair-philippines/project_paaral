"""Helper routines for working with geographic resources and road networks."""
...
class MapResources:
    """Provide access to administrative boundaries and OSM road networks."""
    ...
    def __init__(self, preloaded=True):
        ...
        preloaded : bool, optional
            If ``True`` load cached PSGC shapefiles on creation.
        """
        self.preloaded = preloaded
        self.gadm = self.load_psgc_shapefiles()

    def load_psgc_shapefiles(self):
        """Load Philippines PSGC shapefiles from disk or source files."""
        if self.preloaded == True:
            if os.path.exists(os.path.join(cf.get_path("processed_data"), "psgc_shapefiles.pkl")):
                fpath = os.path.join(cf.get_path("processed_data"), "psgc_shapefiles.pkl")
                ...
                return self._load_preprocess_ph_shapefiles()
                
        elif self.preloaded == False:
            return self._load_preprocess_ph_shapefiles()

    def _load_preprocess_ph_shapefiles(self):
        gdf_ph = gpd.read_file(
            os.path.join(cf.get_path("ph_shapefiles"), "BgySubMuns")
        )
        ...
        gdf["psgc_code"] = (
            gdf["psgc_code"].apply(self._add_leading_zero).astype("string")
        )
        gdf = gdf.rename(
        ...
        gdf["adm2_psgc"] = gdf["adm4_psgc"].apply(
            lambda x: x[:5] + "00000"
        )
        gdf["adm1_psgc"] = gdf["adm4_psgc"].apply(
            lambda x: x[:2] + "00000000"
        )
    ...
    def _add_leading_zero(self, x):
        """Normalize PSGC codes to 10-digit strings."""
        str_x = str(x)
    ...
    def append_psgc_labels(self, gdf_brgys):
        """Attach PSGC name labels to a GeoDataFrame of barangays."""
        # Load the CSV files that accompanied the psgc shapefiles
        dir_csvs = os.path.join(cf.get_path("ph_shapefiles"), "csv")
        ...
        for i, path in enumerate(filepaths):
            ...
            for col in psgc_cols:
                tmp_df[col] = (
                    tmp_df[col].apply(self._add_leading_zero).astype("string")
                )
        ...
        manila_city_psgc3 = tmp_adm3[
            tmp_adm3["adm3_en"]
            .astype("string")
            .str.contains(r"manila", flags=re.IGNORECASE)
        ]["adm3_psgc"].values[0]
    ...
    def get_filepaths_of_regional_road_networks(self):
        """Index pickle files containing regional road networks."""
        dir_path = os.path.join(cf.get_path("ph_roadNetworks"), "regional_drive_graphs") # 
        ...
        pattern = r"(.*?).pkl"
        keys = [
            re.findall(pattern, file, re.IGNORECASE)[0]
    ...
    def inspect_regional_road_network(self, filepath, figsize=(6, 6)):
        """Plot a saved regional road network for visual inspection."""
        start_time = time.time()
        pattern = r"/([^/]+)\.pkl$"
        region_name = re.findall(pattern, filepath)[0]

        print(
            f"Inspecting the preloaded OSMNx road network graph of {region_name}."
        )
    ...
    def preprocess_public_school_coordinates_further(self, gdf_public):
        """Convert valid public school coordinates to geometry objects."""
        pub_tmp = gdf_public.copy()

        mask = (pub_tmp["longitude"].notna()) & (pub_tmp["latitude"].notna())
    ...
    def preprocess_private_school_coordinates_further(self, gdf_private):
        """Validate and convert private school coordinates to geometries."""
        gdf_priv = gdf_private.copy()
        gdf_priv[["longitude", "latitude"]] = gdf_priv[
            ["longitude", "latitude"]
        ].astype("string")

        validate_lon = gdf_priv.loc[:, "longitude"].apply(
            lambda val: self.is_valid_decimal_coordinate(
                val, coordinate_type="longitude"
            )
        )
        validate_lat = gdf_priv.loc[:, "latitude"].apply(
            lambda val: self.is_valid_decimal_coordinate(
                val, coordinate_type="latitude"
            )
        )
        ...
        valid_priv = gpd.GeoDataFrame(
            valid_priv, geometry="geometry", crs=4326
        )
    ...
    def get_adjacent_geographies(self, gpd_gadm, target_psgc, max_depth=2):
        ...
        start_time = time.time()
        print(
            f"Finding adjacent geographies for PSGC {target_psgc} with max_depth={max_depth}"
        )
        ...
        if target_psgc.endswith("0000000"):
            ...
            admin_level = "region"
        elif target_psgc.endswith("00000") and not target_psgc.endswith(
            "0000000"
        ):
        ...
        if not target_indices:
            print(
                f"Warning: No geography found with {psgc_column}={target_psgc}"
            )
            ...
            if not target_indices:
                print(
                    f"No geography found for PSGC {target_psgc} in any column"
                )
                return results
        ...
        for depth in range(1, max_depth + 1):
            ...
            for _, frontier_geo in frontier_geographies.iterrows():
                ...
                buffered_geometry = frontier_geo.geometry.buffer(
                    buffer_tolerance
                )

                # Get candidates using spatial index
                candidate_indices = list(
                    gpd_gadm.sindex.intersection(buffered_geometry.bounds)
                )
                ...
                for idx, candidate in candidates.iterrows():
                    ...
                    if idx in all_found_indices or pd.isna(
                        candidate[psgc_column]
                    ):
                        continue

                    # Check if it's actually adjacent
                    if buffered_geometry.touches(candidate.geometry) or (
                        buffered_geometry.intersects(candidate.geometry)
                        and not frontier_geo.geometry.equals(
                            candidate.geometry
                        )
                    ):
    ...
    def extract_schools_from_geographies(
        self, geography_results, gdf_public=None, gdf_private=None
    ):
        """
        ...
        if gdf_public is not None and (
            not hasattr(gdf_public, "sindex") or gdf_public.sindex is None
        ):
            gdf_public = gdf_public.copy()
            gdf_public.create_sindex()

        if gdf_private is not None and (
            not hasattr(gdf_private, "sindex") or gdf_private.sindex is None
        ):
        ...
        if gdf_public is not None:
            ...
            pub_candidates_idx = list(
                gdf_public.sindex.intersection(target_bounds)
            )
            ...
            public_schools_in_target = pub_candidates.loc[
                in_target_mask
            ].copy()
            results["public_schools_in_target"] = public_schools_in_target
            print(
                f"Found {len(public_schools_in_target)} public schools in target area"
            )
            ...
            for depth, adjacent_shape in adjacent_shapes.items():
                ...
                adjacent_candidates_idx = list(
                    gdf_public.sindex.intersection(adjacent_bounds)
                )
                ...
                public_schools_in_adjacent = adjacent_candidates.loc[
                    in_adjacent_mask
                ].copy()
                results["public_schools_in_adjacent"][
                    depth
                ] = public_schools_in_adjacent
                print(
                    f"Found {len(public_schools_in_adjacent)} public schools in adjacent areas at depth {depth}"
                )
        ...
        if gdf_private is not None and search_shape_complete is not None:
            ...
            print(
                f"Found {len(private_schools_all)} private schools in the entire search area"
            )
            ...
            private_schools_in_target = private_schools_all.loc[
                in_target_mask
            ].copy()
            results["private_schools_in_target"] = private_schools_in_target
            print(
                f"Found {len(private_schools_in_target)} private schools in target area"
            )
            ...
            for depth, adjacent_shape in adjacent_shapes.items():
                ...
                results["private_schools_in_adjacent"][
                    depth
                ] = private_schools_in_adjacent
                print(
    ...
    def is_valid_decimal_coordinate(self, value, coordinate_type="both"):
        ...
        if isinstance(value, (list, tuple)):
            ...
            if len(value) == 2:
                if coordinate_type == "latitude":
                    result = self.is_valid_decimal_coordinate(
                        value[0], "latitude"
                    )
                    ...
                    lat_result = self.is_valid_decimal_coordinate(
                        value[0], "latitude"
                    )
                    lon_result = self.is_valid_decimal_coordinate(
                        value[1], "longitude"
                    )
                ...
                return (
                    False,
                    value,
                    "Coordinate pair must contain exactly 2 values",
                )
        ...
        try:
            ...
            if coordinate_type.lower() == "latitude":
                if -90 <= coord <= 90:
                    ...
                    return (
                        False,
                        coord_,
                        f"{coord} -> {coord_} Note: Longitude must be between -90 and 90 degrees",
                    )
                ...
                if -180 <= coord <= 180:
                    ...
                    return (
                        False,
                        coord_,
                        f"{coord} -> {coord_} Note: Longitude must be between -180 and 180 degrees",
                    )
                ...
                if -90 <= coord <= 90:
                    ...
                    return (
                        False,
                        coord_,
                        f"{coord} -> {coord_} Note: Value must be between -90 and 90 (latitude) or -180 and 180 (longitude)",
                    )
                ...
                return (
                    False,
                    value,
                    "coordinate_type must be 'latitude', 'longitude', or 'both'",
                )
            ...
            return (
                False,
                value,
                f"Could not convert to decimal number: {str(e)}",
            )

    def _handle_excess_decimal_points(self, coord, insert_index):
        """Insert a decimal point at ``insert_index`` within a numeric string."""
