...
This module defines :class:`OptimizedSchoolNetworkBuilder`, a helper for
building distance matrices and shortest paths between public and private
schools.  It converts road data from NetworkX to ``igraph`` and uses
spatial indexing for fast queries.  The builder is designed to operate on
a single master graph so that routes for many schools can be computed
efficiently.
"""
...
class OptimizedSchoolNetworkBuilder:
    ...
    def __init__(self, road_network_graph, public_schools_gdf, private_schools_gdf,
                 peripheral_schools_gdf, admin_boundary):
        ...
        private_schools_gdf : geopandas.GeoDataFrame
            GeoDataFrame of private schools.
        peripheral_schools_gdf : geopandas.GeoDataFrame
            GeoDataFrame of peripheral schools.
        ...
        if self.peripheral_schools.crs != target_crs:
            self.peripheral_schools = self.peripheral_schools.to_crs(target_crs)
    ...
    def build_complete_network(self, buffer_distance_m=5000, max_distance_km=15):
        ...
        results = self._parallel_process_schools(buffer_distance_m, max_distance_km)
    ...
    def _debug_single_school(self):
        ...
        school_row = self.public_schools[self.public_schools['school_id'] == first_school_id].iloc[0]
        nearby_schools = self._find_nearby_schools_fast(school_row.geometry, 5000)
        print(f"Found {len(nearby_schools)} nearby schools: {nearby_schools[:5]}...")
        
        # Test distance calculation
        if nearby_schools:
            distance_result = self._calculate_distances_fast(first_school_id, nearby_schools[:5], [])
    ...
    def _setup_master_infrastructure(self):
        ...
        self._create_school_vertex_mappings()  # ← REMOVE the all_schools parameter
        
        print(f"✅ Master graph: {len(self.master_igraph.vs)} vertices, {len(self.master_igraph.es)} edges")
        print(f"✅ Master graph 4326: {len(self.master_igraph_4326.vs)} vertices")
        print(f"✅ Mapped {len(self.school_to_vertex_map)} schools to road network")
    ...
    def _combine_all_schools(self):
        ...
        print(f"School counts: Public={len(public_std)}, Private={len(private_std)}, Peripheral={len(peripheral_std)}")
    ...
    def _create_master_igraph(self):
        ...
        # Project road network to same CRS as schools
        G_projected = ox.projection.project_graph(self.G_road, to_crs="epsg:3123")
        print(f"Projected graph: {len(G_projected.nodes)} nodes, {len(G_projected.edges)} edges")
        
        # Convert to iGraph using optimized method
        ig_graph = self._nx_to_igraph_optimized(G_projected)
    ...
    def _nx_to_igraph_optimized(self, G):
        ...
        print(f"Created iGraph with {len(ig.vs)} vertices and {len(ig.es)} edges")
        return ig
    
    def _create_school_vertex_mappings(self):  # ← REMOVE the all_schools parameter
        """Map each school to the nearest road network vertex."""
        print("🗺️  Mapping schools to road network vertices...")
        
        # Project the road graph for coordinate matching
        G_proj = ox.projection.project_graph(self.G_road, to_crs="epsg:3123")
        
        successful_mappings = 0
        failed_mappings = 0
        
        # Use self.all_schools_combined instead of the parameter
        for idx, school in self.all_schools_combined.iterrows():  # ← USE self.all_schools_combined
            try:
                # Get school coordinates (already in EPSG:3123)
                if hasattr(school.geometry, 'x'):
                    x, y = school.geometry.x, school.geometry.y
                else:
                    print(f"Warning: School {school.get('school_id', 'unknown')} has invalid geometry")
                    failed_mappings += 1
                    continue
                
                # Find nearest road network node
                nearest_osmid = ox.distance.nearest_nodes(G_proj, x, y)
                
                # Find corresponding vertex in iGraph
                vertex_idx = None
                for i, vertex in enumerate(self.master_igraph.vs):
                    if vertex['osmid'] == nearest_osmid:
                        vertex_idx = i
                        break
                ...
                if vertex_idx is not None:
                    ...
                    self.vertex_to_school_map.setdefault(vertex_idx, []).append(school_id)

                    # attach school info to the vertex
                    v = self.master_igraph.vs[vertex_idx]
                    v['school_ids'] = self.vertex_to_school_map[vertex_idx]
                    prev_types = v['school_types'] if 'school_types' in v.attributes() and v['school_types'] is not None else []
                    prev_names = v['school_names'] if 'school_names' in v.attributes() and v['school_names'] is not None else []
                    prev_attrs = v['school_attrs'] if 'school_attrs' in v.attributes() and v['school_attrs'] is not None else []
                    v['school_types'] = prev_types + [school['school_type']]
                    v['school_names'] = prev_names + [school.get('school_name', '')]
                    v['school_attrs'] = prev_attrs + [self._extract_school_attributes(school)]
                    v['is_school'] = True

                    # replicate attributes in WGS84 graph
                    v4326_idx = None
                    for j, vert in enumerate(self.master_igraph_4326.vs):
                        if vert['osmid'] == nearest_osmid:
                            v4326_idx = j
                            break
                    if v4326_idx is not None:
                        vtx = self.master_igraph_4326.vs[v4326_idx]
                        vtx['school_ids'] = self.vertex_to_school_map[vertex_idx]
                        prev_t = vtx['school_types'] if 'school_types' in vtx.attributes() and vtx['school_types'] is not None else []
                        prev_n = vtx['school_names'] if 'school_names' in vtx.attributes() and vtx['school_names'] is not None else []
                        prev_a = vtx['school_attrs'] if 'school_attrs' in vtx.attributes() and vtx['school_attrs'] is not None else []
                        vtx['school_types'] = prev_t + [school['school_type']]
                        vtx['school_names'] = prev_n + [school.get('school_name', '')]
                        vtx['school_attrs'] = prev_a + [self._extract_school_attributes(school)]
                        vtx['is_school'] = True

                    successful_mappings += 1
                else:
                    failed_mappings += 1
                    print(f"Warning: Could not find vertex for osmid {nearest_osmid}")
                    
            except Exception as e:
                failed_mappings += 1
                print(f"Warning: Could not map school {school.get('school_id', 'unknown')}: {e}")
        ...
        try:
            ...
            for idx, row in self.public_schools.iterrows():
                if hasattr(row.geometry, 'bounds'):
                    ...
                    self.spatial_indices['public'].insert(idx, row.geometry.bounds)
            
            # Private schools index  
            self.spatial_indices['private'] = index.Index()
            for idx, row in self.private_schools.iterrows():
                if hasattr(row.geometry, 'bounds'):
                    self.spatial_indices['private'].insert(idx, row.geometry.bounds)
            
            # Peripheral schools index
            self.spatial_indices['peripheral'] = index.Index()
            for idx, row in self.peripheral_schools.iterrows():
                if hasattr(row.geometry, 'bounds'):
                    self.spatial_indices['peripheral'].insert(idx, row.geometry.bounds)
                    
            print(f"✅ Built spatial indices for {len(self.spatial_indices)} school types")
    ...
    def _parallel_process_schools(self, buffer_distance_m, max_distance_km):
        """Process each origin school sequentially (was parallel).
        ...
        # Get list of origin schools (public schools in target area)
        origin_schools = self.public_schools['school_id'].tolist()
        
        print(f"🔄 Processing {len(origin_schools)} schools in parallel...")
        
        # CHANGE: Use sequential processing instead of multiprocessing to avoid pickling issues
        results = []
        for school_id in origin_schools:
            result = self._process_single_school_optimized(school_id, buffer_distance_m, max_distance_km)
            results.append(result)
        
        return results
    
    def _process_single_school_optimized(self, school_id, buffer_distance_m, max_distance_km):
        """Compute candidate routes for one origin school.
        ...
        try:
            # Get school info
            school_row = self.public_schools[self.public_schools['school_id'] == school_id].iloc[0]
            
            # Find nearby schools using spatial indices
            nearby_schools = self._find_nearby_schools_fast(school_row.geometry, buffer_distance_m)
            
            # Get vertex indices for all relevant schools
            relevant_school_ids = [school_id] + nearby_schools
            vertex_indices = [self.school_to_vertex_map.get(sid) for sid in relevant_school_ids]
            ...
            distance_matrix = self._calculate_distances_fast(school_id, nearby_schools, vertex_indices)
            ...
            # Calculate routes
            routes = self._calculate_routes_fast(school_id, candidates, vertex_indices)
    ...
    def _find_nearby_schools_fast(self, origin_geometry, buffer_distance_m):
        ...
        for school_type, gdf in school_gdfs:
            if school_type in self.spatial_indices:
                # Use spatial index
                candidates = list(self.spatial_indices[school_type].intersection(bounds))
    ...
    def _calculate_distances_fast(self, origin_school_id, destination_school_ids, vertex_indices):
        ...
        vertex_indices : list
            List of igraph vertex indices corresponding to all schools.
        ...
        if origin_vertex is None:
            print(f"Warning: Origin school {origin_school_id} not mapped to vertex")
            return pd.DataFrame()
        ...
        for dest_id in destination_school_ids:
            ...
            if dest_vertex is not None and dest_vertex not in seen_vertices:
                ...
                print(f"Skipping duplicate vertex {dest_vertex} for school {dest_id}")
        
        if not dest_vertices:
            print(f"Warning: No valid destination vertices found for school {origin_school_id}")
            return pd.DataFrame()
        
        print(f"Calculating distances from {origin_school_id} to {len(dest_vertices)} unique destinations")
        ...
        try:
            ...
            print(f"Distance calculation error for school {origin_school_id}: {e}")
            return pd.DataFrame()
    ...
    def _get_candidates_fast(self, distance_matrix):
        ...
        valid_destinations = distance_matrix.columns[distance_matrix.iloc[0].notna()].tolist()
        
        return {origin_id: valid_destinations}
    
    def _calculate_routes_fast(self, origin_school_id, candidates_dict, vertex_indices):
        ...
        vertex_indices : list
            Vertex indices corresponding to schools involved.
        ...
        dest_vertices = [self.school_to_vertex_map.get(dest_id) for dest_id in candidates]
        ...
        if dest_vertices:
            try:
                ...
                for i, dest_id in enumerate(candidates):
                    if i < len(paths) and self.school_to_vertex_map.get(dest_id) is not None:
                        routes[dest_id] = paths[i]
                    else:
                        routes[dest_id] = []
                        
            except Exception as e:
                print(f"Route calculation error for school {origin_school_id}: {e}")
    ...
    def _consolidate_results(self, results):
        ...
        if all_distance_matrices:
            consolidated_distances = pd.concat(all_distance_matrices, ignore_index=False)
        else:
            consolidated_distances = pd.DataFrame()
        
        print(f"✅ Consolidated {len(all_distance_matrices)} distance matrices")
    ...
    def _create_master_igraph_4326(self):
        ...
        if hasattr(self.G_road, 'graph') and 'crs' in self.G_road.graph:
            if self.G_road.graph['crs'] != 'epsg:4326':
                G_4326 = ox.projection.project_graph(self.G_road, to_crs="epsg:4326")
            else:
    ...
    def _create_pruned_school_graph(self, distance_df):
        """Create an igraph of public and private schools with valid distances."""
        ...
        for origin_id, row in distance_df.iterrows():
            ...
            for dest_id, dist in row.dropna().items():
                if dest_id in public_ids or dest_id in private_ids:
        ...
        if edges:
            edge_pairs = [(name_to_idx[o], name_to_idx[d]) for o, d, _ in edges]
