"""Utilities for loading and preprocessing educational datasets."""
...
class KnowledgeBase:
    """Container class for school datasets used throughout the project."""

    def __init__(self, load_knowledge_base=False):
        """Initialize the knowledge base and optionally load data.

        Parameters
        ----------
        load_knowledge_base : bool, optional
            If ``True`` the datasets are loaded immediately.
        """
    ...
    def load_knowledge_base(self):
        ...
        concat_paths = lambda directory, filename: os.path.join(cf.get_path(directory), filename)

        print("Loading public school coordinates as of SY 2023-2024.")
        path_public_coordinates = concat_paths("public_data", "SY 2023-2024 LIST OF SCHOOLS WITH LONGITUDE AND LATITUDE.xlsx")
        self.public_coordinates = self.use_talino_public_school_coordinates(
            fpath=path_public_coordinates,
        )
        print(
            f"Time elapsed for public school coordinates: {(time.time()-load_stime):.2f} seconds\n"
        )
        load_stime = time.time()

        print("Loading private school coordinates as of 2024.")
        path_private_coordinates = concat_paths("processed_data", "raw_validation_sheets")
        self.private_coordinates = self.set_private_coordinates(
            dirpath=path_private_coordinates
        )
        print(
            f"Time elapsed for private school coordinates: {(time.time()-load_stime):.2f} seconds\n"
        )
        load_stime = time.time()
        ...
        path_enrollment = concat_paths("public_data", "Copy of SY 2023-2024 SCHOOL LEVEL DATA ON ENROLLMENT.csv")
        self.enrollment_info, self.enrollment = (
            self.load_preprocess_enrollment(
                fpath=path_enrollment,
        ...
        print(
            f"Time elapsed for enrollment & SHS offerings: {(time.time()-load_stime):.2f} seconds\n"
        )
        load_stime = time.time()
        ...
        path_public_seats = concat_paths("public_data", "SY 2023-2024 SEAT-LEARNER RATIO.xlsx")
        self.public_seats_info, self.public_seats = (
            self.load_preprocess_public_seats(
                fpath=path_public_seats,
                sheet_name="DATABASE",
            )
        )

        path_private_seats = concat_paths("private_data", "priv_classroom_furniture.xlsx")
        self.private_seats_info, self.private_seats = (
            self.load_preprocess_private_seats(
                fpath=path_private_seats
            )
        )
        print(
            f"Time elapsed for public & private seats: {(time.time()-load_stime):.2f} seconds\n"
        )
        load_stime = time.time()

        print("Loading public school shifting schedule for SY 2023-2024.")
        path_shifting = concat_paths("public_data", "SY 2022-2023 LIST OF PUBLIC SCHOOLS WITH SHIFTING SCHEDULE.xlsx")
        ...
        print(
            f"Time elapsed for public shifting: {(time.time()-load_stime):.2f} seconds\n"
        )
        load_stime = time.time()
        ...
        path_gastpe_schools = concat_paths("private_data", "2024-2025 GASTPE Participating Schools Top-up Data.xlsx")
        self.esc, self.shsvp = self.load_esc_amounts(
            fpath=path_gastpe_schools
        )
        path_gastpe_data = concat_paths("private_data", "ESC and SHSVP Tuition.xlsx")
        self.gastpe = self.load_gastpe_dataset(
            fpath=path_gastpe_data
        )
        print(
            f"Time elapsed for public shifting: {(time.time()-load_stime):.2f} seconds\n"
        )
    ...
    def use_talino_public_school_coordinates(self, fpath):
        """Load preprocessed public school coordinates from an Excel file.

        Parameters
        ----------
        fpath : str
            Path to the Excel file containing coordinates.

        Returns
        -------
        pandas.DataFrame
            DataFrame with standardized column names and ``school_id``.
        """
        df_excel = pd.read_excel(fpath, sheet_name="DB", header=5)

        df = df_excel.copy()
        df.columns = [
            "_".join(str(col).strip().lower().split()) for col in df.columns
        ]
        ...
        df = df.rename(columns={"lis_school_id": "school_id"})

        return df

    def set_public_school_coordinates(self, fpath_nsbi, fpath_osm):
        """Merge NSBI and OSM coordinate datasets for public schools.

        Parameters
        ----------
        fpath_nsbi : str
            CSV file with NSBI data.
        fpath_osm : str
            Shapefile or GeoPackage with OSM data.

        Returns
        -------
        pandas.DataFrame
            Merged coordinate table indexed by string IDs.
        """
        df_nsbi = self.load_preprocess_nsbi(fpath_nsbi)
        gdf_osm = self.load_preprocess_osm(fpath_osm)

        df_merge = df_nsbi.merge(
            gdf_osm, left_on="lis school id", right_on="ref", how="left"
        )
    ...
    def load_preprocess_nsbi(self, fpath):
        """Preprocess the NSBI coordinate CSV for merging with OSM data.

        Parameters
        ----------
        fpath : str
            Path to the NSBI CSV file.
    ...
    def load_preprocess_osm(self, fpath):
        """Preprocess OSM-derived public school coordinates.

        Parameters
        ----------
        fpath : str
            Path to the geospatial file read by ``geopandas``.

        Returns
        -------
        geopandas.GeoDataFrame
            Cleaned dataset with columns ``ref``, ``longitude_osm`` and ``latitude_osm``.
        """
        ...
        dtmp_osm["longitude_osm"] = dtmp_osm["geometry"].apply(
            lambda x: self._get_coord(x)
        )
        dtmp_osm["latitude_osm"] = dtmp_osm["geometry"].apply(
            lambda x: self._get_coord(x, longitude=False)
        )
    ...
    def _get_coord(self, x, longitude=True):
        """Return either the x or y coordinate of a geometry."""
        coord = 0
    ...
    def _handle_duplicate_osm_coordinates(self, gdf_osm):
        """Dissolve duplicate OSM features belonging to the same school."""
        gdf = gdf_osm.copy()
    ...
    def set_private_coordinates(self, dirpath):
        """Load and concatenate private school coordinate sheets.

        Parameters
        ----------
        dirpath : str
            Directory containing Excel files for each region.

        Returns
        -------
        pandas.DataFrame
            Combined dataframe of private school coordinates.
        """
        ...
        for path in files[1:]:
            ...
            for sheet in sheets.keys():
                ...
                headers_ = [
                    header.strip() for header in headers if type(header) == str
                ]
                tdf = tdf[headers_]
        ...
        regions_df["Modified COC"] = regions_df["Modified COC"].replace(
            replace_coc
        )
        ...
        regions_df = regions_df.rename(columns={"beis school id": "school_id"})
    ...
    def load_preprocess_enrollment(
        ...
        """Load and normalize enrollment spreadsheets.

        Parameters
        ----------
        fpath : str
            Path to the enrollment CSV or Excel file.
        excel_sheet_name : str, optional
            Sheet name when ``fpath`` is an Excel workbook.
        split_header : str, optional
            Column name used to split info and enrollment values.
        non_graded : dict, optional
            Flags indicating whether to include non-graded levels.

        Returns
        -------
        tuple[pandas.DataFrame, pandas.DataFrame]
            Tuple of ``(info_df, enrollment_df)``.
        """
        if ".xlsx" in fpath:
        ...
        index = [
            ...
            if re.search(r"^Region$", str(val).strip())
        ][0]
        ...
        split_column = [
            col for col in pdf.columns if split_header in col.lower()
        ][0]
        ...
        to_numeric = lambda val: (
            np.nan if val == "-" else int(val.replace(",", ""))
        )
        ...
        category_enc, lvl_educ = self._build_alternate_categories(
            category_vals, non_graded=non_graded
        )
    ...
    def _build_alternate_categories(self, category_vals, non_graded):
        """Generate alternative category labels for enrollment data."""
        sexes = ["Male", "Female"]
        ...
        category_enc = {
            cat: alt for cat, alt in zip(category_vals, alt_category)
        }

        return category_enc, level_of_educ

    def extract_shs_offerings(self, enrollment):
        """Derive SHS strand availability from enrollment records."""
        ...
        for col in shs_offerings.columns:
            shs_offerings[col] = shs_offerings[col].apply(
                lambda x: 1 if x >= 1 else 0
            )

        return shs_offerings

    def load_preprocess_public_seats(
        self, fpath, sheet_name, split_header="modified"
    ):
        """Load furniture counts for public schools."""
        ...
        coc_column = [
            col for col in columns if split_header in col.strip().lower()
        ][0]
        coc_idx = [
            i
            for i, col in enumerate(columns)
            if split_header in col.strip().lower()
        ][0]
        df_info = pdf.loc[:, :coc_column].copy()

        pattern = r"^(?!.*mother).*school[_ ]id$"
        id_column = [
            col for col in columns if re.search(pattern, col, re.IGNORECASE)
        ][0]

        pdf_trim = pd.concat(
            [pdf.iloc[:, : coc_idx + 1], pdf.iloc[:, coc_idx + 1 + 5 : -4]],
            axis=1,
        )
    ...
    def load_preprocess_private_seats(
        self, fpath, sheet_name="priv_classroom_furniture"
    ):
        """Preprocess classroom furniture counts for private schools."""
        ...
        split_ix = [
            ix for ix, col in enumerate(df_fr.columns) if col == split_col
        ][0]
        ...
        pattern = r"chairs"
        mask_seats = (
            fr_melt["furniture_category"].str.contains(
        ...
        for pattern, level in zip(patterns, levels):
            mask = fr_seats["furniture_category"].str.contains(
                pattern, regex=True, flags=re.IGNORECASE
            )
            fr_seats.loc[mask, "level_of_education"] = [
                level for _ in range(fr_seats.loc[mask].shape[0])
            ]
    ...
    def load_preprocess_public_shifting(
        self, fpath, excel_sheet_name="DB", split_header="shifting"
    ):
        """Load public school shifting schedule information."""
    ...
    def load_esc_amounts(self, fpath):
        """Load ESC and SHS VP payment tables from an Excel workbook."""
        ...
        for i, df in enumerate(dfs):
            ...
            df.columns = [
                "_".join(col.strip().lower().split()) for col in df.columns
            ]
    ...
    def load_gastpe_dataset(self, fpath, sheet_name="Tuition"):
        """Load GASTPE tuition tables and merge ESC amounts."""
        ...
        fpath = os.path.join(
            cf.get_path("private_data"),
            "ESC and SHSVP Tuition.xlsx"
        ) # "../datasets/private/ESC and SHSVP Tuition.xlsx"
        ...
        df_gastpe.columns = [
            "_".join(col.split(" ")) for col in df_gastpe.columns
        ]

        df_gastpe["deped_school_id"] = df_gastpe["deped_school_id"].astype(
            "string"
        )
        df_gastpe["program"] = df_gastpe["program"].astype("string")
        df_gastpe["suc/luc"] = [
            1 if str(val) == "SUC/LUC" else 0 for val in df_gastpe["suc/luc"]
        ]

        program_esc = [
            1 if str(val).lower() in ["esc", "both"] else 0
            for val in df_gastpe["program"].values
        ]
        program_shsvp = [
            1 if str(val).lower() in ["shs", "both"] else 0
            for val in df_gastpe["program"].values
        ]
        ...
        df_gastpe["esc_school_id"] = [
            str(int(val)) if str(val) not in ["NaN", "nan"] else "None"
            for val in df_gastpe["esc_school_id"].values
        ]
        ...
        df_gastpe = df_gastpe.merge(
            old_gastpe,
            left_on="esc_school_id",
            right_on="esc_school_id_ref",
            how="left",
        )

        # Drop irrelevant columns
        ir_cols = [
            "billed_in_sy_2024-2025_(esc)",
            "program",
            "esc_school_id_ref",
            "region_shs",
        ]
        rename_cols = {"deped_school_id": "school_id"}
        df_gastpe = (
            df_gastpe.drop(columns=ir_cols)
    ...
    def compile_public_datasets(self):
        """Merge all processed public school datasets into a single table."""
        ...
        pub_seats_info, pub_seats = (
            self.public_seats_info.copy(),
            self.public_seats.copy(),
        )
        ...
        enr_info_ = enr_info[["school_id", "modified coc"]].set_index(
            "school_id"
        )
        ...
        shs_offerings.columns = [
            "shs_" + str(col) for col in shs_offerings.columns
        ]
        ...
        compiled_public["sector"] = [
            "Public" for _ in range(compiled_public.shape[0])
        ]

        return compiled_public

    def compile_private_datasets(self):
        """Merge all processed private school datasets into a single table."""
        # Enrollment of public and private
        enr_info = self.enrollment_info.copy()
        enr = self.enrollment.copy()
        shs_offerings = self.shs_offerings.copy()
        ...
        priv_enr_pvt.columns = [
            "enrollment_es",
            "enrollment_jhs",
            "enrollment_shs",
        ]

        private_seats_info, private_seats = (
            self.private_seats_info.copy(),
            self.private_seats.copy(),
        )
        ...
        compiled_private = compiled_private.rename(
            columns={"school name": "school_name"}
        )
        # BANDAID solution to "sector"
        compiled_private["sector"] = [
            "Private" for _ in range(compiled_private.shape[0])
        ]
